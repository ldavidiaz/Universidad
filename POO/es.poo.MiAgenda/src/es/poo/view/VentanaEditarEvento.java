...
public class VentanaEditarEvento extends JFrame{
	...
	public void setInputDesdeHora(JSpinner inputDesdeHora) {
		this.inputDesdeHora = inputDesdeHora;
	}

	public void setInputHastaHora(JSpinner inputHastaHora) {
		this.inputHastaHora = inputHastaHora;
	}

	private JTextField txtFieldTituloEvento;
	private JTextArea txtAreaDescripcion;	
	private JButton btnActualizarEvento,btnVolverEvento;
	private JDateChooser inputFechaDesde, inputFechaHasta;
	
	Fuente tipoFuente;
	
	private JLabel lblNotificar;
	private JComboBox <String> cbNotificarSiNo;
	...
	//VENTANA VER CONTACTOS AÑADIDOS
	...
	public void setObjNotificacion(ArrayList<Notificacion> objNotificacion) {
	...
	public VentanaEditarEvento(VentanasController controllerVentanas) {
		...
		//BOTON PARA AÑADIR MÁS DETALLES DEL EVENTO
		...
		//SETEAR EL MODELO CUANDO SE HAYA ELEGIDO LA OPCION 'CUMPLEAÑOS' DEL COMBOBOX
		...
		btnActualizarEvento = new JButton("ACTUALIZAR");
		btnActualizarEvento.setOpaque(true);
		btnActualizarEvento.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		btnActualizarEvento.setBorder(null);
		btnActualizarEvento.setBackground(new Color(0, 51, 0));
		btnActualizarEvento.setForeground(new Color(255, 255, 255));
		btnActualizarEvento.setHorizontalAlignment(SwingConstants.CENTER);
		btnActualizarEvento.setBounds(400, 515, 90, 25);
		btnActualizarEvento.setFont(tipoFuente.fuente(tipoFuente.MINECRAFT, 0, 14));
		btnActualizarEvento.addActionListener(controllerEvento);
        ...
        /*.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (indiceSeleccionado != -1) {
                	Notificacion notificacionSeleccionada = listaNotificaciones.get(indiceSeleccionado);
                    listaNotificaciones.remove(indiceSeleccionado);
	...
	public JButton getBtnAñadirContacto() {
		return btnActualizarEvento;
	}
	public JButton getBtnActualizarEvento() {
		return btnActualizarEvento;
	}
	...
	public void setInputFechaDesde(String fechaDesde) {
        SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
        try {
            Date fecha = formatter.parse(fechaDesde);
            this.inputFechaDesde.setDate(fecha);
        } catch (ParseException e) {
            e.printStackTrace();
        }
	}
	public void setInputFechaHasta(String fechaHasta) {
        SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
        try {
            Date fecha = formatter.parse(fechaHasta);
            this.inputFechaHasta.setDate(fecha);
        } catch (ParseException e) {
            e.printStackTrace();
        }
	}
	public void setInputFechaAniversario(String fechaA) {
        SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");
        try {
            Date fecha = formatter.parse(fechaA);
            this.inputCitaFechaAniversario.setDate(fecha);
        } catch (ParseException e) {
            e.printStackTrace();
        }
	}
	public void setInputHora(String valor, String desdeOhasta) {
	    SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm");
	    SpinnerDateModel timeModel;
	    try {
	        Date date = dateFormat.parse(valor);

	        if (desdeOhasta.equals("DESDE")) {
	            timeModel = new SpinnerDateModel(date, null, null, Calendar.HOUR_OF_DAY);
	            inputDesdeHora.setModel(timeModel);
		        JSpinner.DateEditor de_inputDesdeHora = new JSpinner.DateEditor(inputDesdeHora, "HH:mm");
		        inputDesdeHora.setEditor(de_inputDesdeHora);
	        } else if (desdeOhasta.equals("HASTA")) {
	            timeModel = new SpinnerDateModel(date, null, null, Calendar.HOUR_OF_DAY);
	            inputHastaHora.setModel(timeModel);
		        JSpinner.DateEditor de_inputHastaHora = new JSpinner.DateEditor(inputHastaHora, "HH:mm");
		        inputHastaHora.setEditor(de_inputHastaHora);
	        }
	    } catch (ParseException e) {
	...
	/**
	 * Este método se encarga de inicializar los datos de un evento a partir de un ID.
	 * @param id El ID del evento.
	 */
	public void inicDatos(String id) {
		int idEvento = Integer.parseInt(id);
		btnActualizarEvento.setName(id);
		Cita cita;
		Reunion reunion;
		for (Evento evento : DBAgenda.getListaEventos()) {
			if(evento.getId() == idEvento) {
				...
				String finFecha = evento.getFinFechaEvento();
				String horaHasta = evento.getHoraHasta();
				String notificar = evento.getNotificar();
				ArrayList<Notificacion> notis = evento.getNotificaciones();
				
				inicDatosComunes(titulo,descripcion,fechaEvento,
						horaDesde,finFecha,horaHasta,notificar,notis);
				//controlar visibilidad paneles y botones
				if(notificar.equals("NO")) {
					this.btnAddNotificacion.setVisible(false);
					this.btnVerNotificaciones.setVisible(false);
				}
				else if(notificar.equals("SI")) {
					this.btnAddNotificacion.setVisible(true);
					this.btnVerNotificaciones.setVisible(true);
				}
				if(evento instanceof Cita) {
					cita = (Cita) evento;	
					btnAgregarTipoEvento.setVisible(false);
					btnBorrarTipoEvento.setVisible(true);
					panelTipoEvento.setVisible(true);
					cbTipoEvento.setSelectedItem("CITA");
					String tipoCita = cita.getTipoCita();
					String fechaFormateada = null;
					String cp = null;
					String ubi = null;
					String tipoEspecialidad = null;
					if(tipoCita.equals("CUMPLEAÑOS")) {
						cbTipoCita.setSelectedItem("CUMPLEAÑOS");
						controllerEvento.cargarContactosPersonales();
						controllerEvento.setContactosPersonales("EDITAR","CUMPLE");
						cp = cita.getContactoPersonal();
						inicDatosCitaCumple(cp);
					}
					else if(tipoCita.equals("ANIVERSARIO")) {
						cbTipoCita.setSelectedItem("ANIVERSARIO");
						controllerEvento.cargarContactosPersonales();
						controllerEvento.setContactosPersonales("EDITAR","ANIVERSARIO");
						cp = cita.getContactoPersonal();
						fechaFormateada = cita.getFechaAniversario();
						inicDatosCitaAniversario(cp,fechaFormateada);
					}
					else if(tipoCita.equals("MEDICO")) {
						cbTipoCita.setSelectedItem("MEDICO");
						ubi = cita.getUbicacion();
						tipoEspecialidad = cita.getEspecialidadMedica();
						inicDatosCitaMedico(ubi,tipoEspecialidad);
					}	
				}
				else if(evento instanceof Reunion) {
					reunion = (Reunion) evento;
					btnAgregarTipoEvento.setVisible(false);
					btnBorrarTipoEvento.setVisible(true);
					panelTipoEvento.setVisible(true);
					cbTipoEvento.setSelectedItem("REUNION");
					panelReunionContactos.setVisible(true);
					controllerEvento.cargarContactosReunion("EDITAR");
					ArrayList<String> listaContactos = reunion.getListaContactos();
					addContactosReunion(listaContactos);
					String ubi = null;
					String sala = null;
					...
					String tipoReunion = reunion.getTipoReunion();
					if(tipoReunion.equals("PRESENCIAL")) {
						ubi = reunion.getUbicacion();
						sala = reunion.getSalaReunion();
						inicDatosReunionPresencial(ubi,sala);
					}
					else if(tipoReunion.equals("LLAMADA")) {
						telCorto = reunion.getTelefonoCorto();
						telLargo = reunion.getTelefonoLargo();
						clave = reunion.getClaveAcceso();
						inicDatosReunionLlamada(telCorto,telLargo,clave);
					}
					else if(tipoReunion.equals("VIDEOCONFERENCIA")) {
						tipoVideo = reunion.getTipoVideoconferencia();
						otroTipo = reunion.getOtroTipo();
						inicDatosReunionVideo(tipoVideo,otroTipo);
					}			
				}
				else {
					panelTipoEvento.setVisible(false);
	...
	/**
	 * Este método se encarga de agregar contactos a la reunión.
	 * @param contactos La lista de contactos a agregar.
	 */
	private void addContactosReunion(ArrayList<String> contactos) {
		if(contactos.size()>0) {
			for(String contacto : contactos) {
				listModelContactosReunion.addElement(contacto);
			}
	...
	/**
	 * Este método se encarga de inicializar los datos de una reunión en el panel de videoconferencia.
	 * @param tipo El tipo de videoconferencia.
	 * @param otro El otro tipo de videoconferencia.
	 */
	private void inicDatosReunionVideo(String tipo, String otro) {
		panelReunionVideo.setVisible(true);
		cbEspecialidadMed.setSelectedItem(tipo);
		txtOtraVideoConferencia.setText(otro);	
	}
	/**
	 * Este método se encarga de inicializar los datos de una reunión en el panel de llamada.
	 * @param corto El número de teléfono corto.
	 * @param largo El número de teléfono largo.
	 * @param clave La clave de acceso.
	 */
	private void inicDatosReunionLlamada(String corto,String largo,String clave) {
		panelReunionLlamada.setVisible(true);
		txtTelefonoCorto.setText(corto);
		txtTelefonoLargo.setText(largo);
		txtClaveAcceso.setText(clave);
	}
	/**
	 * Este método se encarga de inicializar los datos de una reunión en el panel presencial.
	 * @param ubi La ubicación de la reunión.
	 * @param sala La sala de la reunión.
	 */
	private void inicDatosReunionPresencial(String ubi,String sala) {
		panelReunionPresencial.setVisible(true);
		txtUbicacionReunion.setText(ubi);
		cbSalaReunion.setSelectedItem(sala);
	}
	/**
	 * Este método se encarga de inicializar los datos de una cita médica.
	 * @param ubicacion La ubicación de la cita médica.
	 * @param especialidad La especialidad médica de la cita.
	 */
	private void inicDatosCitaMedico(String ubicacion, String especialidad) {
		panelCitaMedico.setVisible(true);
		txtUbicacionMed.setText(ubicacion);
		cbEspecialidadMed.setSelectedItem(especialidad);	
	}
	/**
	 * Este método se encarga de inicializar los datos de una cita de aniversario.
	 * @param cp El contacto personal relacionado con la cita.
	 * @param fecha La fecha de aniversario.
	 */
	private void inicDatosCitaAniversario(String cp, String fecha) {
		panelCitaAniversario.setVisible(true);
		for(Contacto contacto : DBAgenda.getListaContactos()) {
			String nombreApe;
			String name = contacto.getNombre().trim();
			String apellido = contacto.getApellidos().trim();
			nombreApe= name + " " + apellido;
			if(nombreApe.equals(cp)) {
				cbContactoAniversario.setSelectedItem(cp);	
			}
		}
		setInputFechaAniversario(fecha);
	}
	/**
	 * Este método se encarga de inicializar los datos de una cita de cumpleaños.
	 * @param cp El contacto personal relacionado con la cita.
	 */
	private void inicDatosCitaCumple(String cp) {
		//comprobar si el contacto existe en la BD
		panelCitaCumple.setVisible(true);
		for(Contacto contacto : DBAgenda.getListaContactos()) {
			String nombreApe;
			String name = contacto.getNombre().trim();
			String apellido = contacto.getApellidos().trim();
			nombreApe= name + " " + apellido;
			if(nombreApe.equals(cp)) {
				cbContactoCumple.setSelectedItem(cp);	
			}
		}
	}
	/**
	 * Este método se encarga de inicializar los datos comunes de un evento.
	 * @param titulo El título del evento.
	 * @param descripcion La descripción del evento.
	 * @param fechaEvento La fecha del evento.
	 * @param horaDesde La hora de inicio del evento.
	 * @param finFecha La fecha de finalización del evento.
	 * @param horaHasta La hora de finalización del evento.
	 * @param notificar La opción de notificación del evento.
	 * @param notis La lista de notificaciones del evento.
	 */
	private void inicDatosComunes(String titulo, String descripcion, String fechaEvento, String horaDesde,
			String finFecha, String horaHasta, String notificar, ArrayList<Notificacion> notis) {
		getTxtFieldTituloEvento().setText(titulo);
		getTxtAreaDescripcion().setText(descripcion);
		setInputFechaDesde(fechaEvento);
		setInputHora(horaDesde, "DESDE");
		setInputFechaHasta(finFecha);
		setInputHora(horaHasta, "HASTA");
		cbNotificarSiNo.setSelectedItem(notificar);
		this.objNotificacion = notis;		
	}
