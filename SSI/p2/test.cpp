...
void bruteForce(std::string texto,int tamKey,int tipoClave);
...
int scoped_main(int argc, char *argv[])
{
	...
	try
	{
		...
		else if (command == "des_DES")
		{
			...
			//plaintextStr.c_str()
			std::string decrypted = DecryptString(plaintext, passPhrase);
			std::cout << "\nDecrypted: " << decrypted << std::endl;
			/*
			std::string ciphertext = EncryptString(plaintext, passPhrase);
			std::cout << "\nCiphertext: " << ciphertext << std::endl;
		...
		else if (command == "enc_DES_fich")
		{
			// VC60 workaround: use char array instead of std::string to workaround MSVC's getline bug
			char passPhrase[MAX_PHRASE_LENGTH];
			std::string ficheroEntrada,ficheroSalida;
			std::cout << "Passphrase: ";
			std::cin.getline(passPhrase, MAX_PHRASE_LENGTH);
			std::cout << "Fichero de entrada a encriptar: ";
			std::cin >> ficheroEntrada;
			std::cout << "Fichero de salida para guardado: ";
			std::cin >> ficheroSalida;
			...
			std::string linea;
			std::string texto;//GUARDAMOS EL TEXTO DEL ARCHIVO
			// Lee y concatena cada línea del archivo en un solo string
			while (std::getline(archivoEntrada, linea)) {
				texto += linea;
				texto += '\n'; // Agrega un salto de línea después de cada línea leída
			}
			const char* plaintext = texto.c_str();
			...
			std::ofstream archivoSalida(ficheroSalida, std::ofstream::app);
			...
			// El string a guardar en el archivo
			// Escribe el string en el archivo
			archivoSalida << ciphertext;
		...
		else if (command == "des_DES_fich")
		{
			...
			std::cout << "Contrasena: ";
			std::cin.getline(passPhrase, MAX_PHRASE_LENGTH);
			std::cout << "Fichero de entrada a desencriptar: ";
			std::cin >> ficheroEntrada;
			std::cout << "Fichero de salida para guardado: ";
			std::cin >> ficheroSalida;
			...
			std::string decrypted = DecryptString(texto.c_str() , passPhrase);
			std::cout << "\nDecrypted: " << decrypted << std::endl;
			std::ofstream archivoSalida(ficheroSalida, std::ofstream::app);
			...
			*/
		...
		else if (command == "fuerzaB") {

			double PCFreq = 0.0;
			__int64 CounterStart = 0;
			//StartCounter(PCFreq, CounterStart); //aqui se tenemos que empezar a contar desde la entrada de datos
			
			//pasamos los datos? fichero,texto,lon key, tipo key
			std::string ficheroEntrada;
			std::cout << "Ingrese fichero de entrada: ";
			std::cin >> ficheroEntrada;
			std::cout << std::endl;


			int tipoClave;
			std::cout << "Tipo de clave: \n-[1] Alfabeticas\n-[2] Alfanumericas" << std::endl;
			std::cin >> tipoClave;

			
			int tamKey;
			std::cout << "Ingrese el tama\xA4o de la clave: ";
			std::cin >> tamKey;
			std::cout << std::endl;
			std::ifstream archivoEntrada(ficheroEntrada);
			...
			bruteForce(texto,tamKey,tipoClave);
			double timeSecs = GetCounter(PCFreq, CounterStart);
			double timeMs = timeSecs * 1000.0;
			std::cout << "Tiempo transcurrido: " << timeSecs << " segundos." << std::endl;
			std::cout << "Tiempo transcurrido: " << timeMs << " milisegundos." << std::endl;
		...
		else if (command == "a")
		{
			...
			std::string keyAES;//GUARDAMOS EL TEXTO DEL ARCHIVO
			// Lee y concatena cada línea del archivo en un solo string
			while (std::getline(keyFile, linea)) {
				keyAES += linea;
				keyAES += '\n'; // Agrega un salto de línea después de cada línea leída
			}
			...
			//BLOCK
			linea = "";
			std::ifstream blockFile(argv[3]);

			// Verifica si se abrió correctamente
			if (!blockFile.is_open()) {
				std::cerr << "No se pudo abrir el archivo." << argv[3] << std::endl;
				return 1;
			}
			std::string blockAES;//GUARDAMOS EL TEXTO DEL ARCHIVO
			// Lee y concatena cada línea del archivo en un solo string
			while (std::getline(blockFile, linea)) {
				blockAES += linea;
				blockAES += '\n'; // Agrega un salto de línea después de cada línea leída
			}
			/*
			...
			// Verifica si se abrió correctamente
			if (!inFile.is_open()) {
				std::cerr << "No se pudo abrir el archivo." << argv[4] << std::endl;
				return 1;
			}
			std::string inputFile;//GUARDAMOS EL TEXTO DEL ARCHIVO
			// Lee y concatena cada línea del archivo en un solo string
			...
			*/
			...
			AES_CTR_Encrypt(keyAES.c_str(), blockAES.c_str(), argv[4], argv[5]);

		}
		else if (command == "des_AES") {
			std::string linea;
			//KEY 
			std::ifstream keyFile(argv[2]);
			...
			AES_CTR_Decrypt(keyAES.c_str(), blockAES.c_str(), argv[4], argv[5]);
		}//modificar para mostrar usage2.dat
...
void bruteForce(std::string texto,int tamKey,int tipoClave) {
							...
							if (k > 3 && keyEncontrada == false) {
								for (int z = 0; z < strlen(alfabeto); z++) {
									...
									try
									{
										...
										std::cout << "\rTrying  Key: " << key;
										std::string decrypted = DecryptString(texto.c_str(), key);
										//llega a la linea de abajo si y solo si DecryptString encuentra la key
										keyEncontrada = true;
										textoClaro = decrypted;
										//break;
										goto found;
									}
									catch (const std::exception&)
									{
										//std::cout << "\rNO VALID KEY";
										delete[] key;
										key = nullptr;
									}
									/*
								...
								try
								{
									...
									//la siguiente linea deberia lanzar un exception si la key no es correcta
									std::cout << "\rTrying  Key: " << key;
									std::string decrypted = DecryptString(texto.c_str(), key);
									textoClaro = decrypted;
									keyEncontrada = true;
	...
	if (keyEncontrada) {
		std::cout << "\nClave encontrada: " << key << std::endl;
		std::cout << "Texto descifrado: " << textoClaro << std::endl;
	}
	else {
		std::cout << "\nNo se ha podido encontrar la clave." << std::endl;
	}
	std::cout << "\nNumero de claves intentadas: " << contador << std::endl;
...
/*
void AES_CTR_Decrypt( const char* key_file, const char* iv_file, const char* input_file, const char* output_file) {
	// Abrir el archivo de entrada (mensaje cifrado)
	std::ifstream input_fp(input_file, std::ios::binary);
	if (!input_fp.is_open()) {
	...
	std::ifstream key_fp(key_file, std::ios::binary);
	if (!key_fp.is_open()) {
		std::cerr << "Error al abrir el archivo de clave" << std::endl;
		exit(EXIT_FAILURE);
	}

	// Abrir el archivo de vector de inicialización
	std::ifstream iv_fp(iv_file, std::ios::binary);
	if (!iv_fp.is_open()) {
		std::cerr << "Error al abrir el archivo de vector de inicialización" << std::endl;
		exit(EXIT_FAILURE);
	}

	// Leer la clave
	SecByteBlock key(AES::DEFAULT_KEYLENGTH);
	key_fp.read(reinterpret_cast<char*>(key.data()), key.size());
	if (key_fp.gcount() != AES::DEFAULT_KEYLENGTH) {
		std::cerr << "Error al leer la clave" << std::endl;
		exit(EXIT_FAILURE);
	}

	// Leer el vector de inicialización
	byte iv[AES::BLOCKSIZE];
	iv_fp.read(reinterpret_cast<char*>(iv), AES::BLOCKSIZE);
	if (iv_fp.gcount() != AES::BLOCKSIZE) {
		std::cerr << "Error al leer el vector de inicialización" << std::endl;
		exit(EXIT_FAILURE);
	}

	// Configurar el cifrador AES en modo CTR
	CTR_Mode<AES>::Decryption aes_decryptor(key, key.size(), iv);

	// Procesar el mensaje cifrado bloque por bloque
	std::string decrypted_text;
	char buffer[AES::BLOCKSIZE];
	while (!input_fp.eof()) {
		input_fp.read(buffer, AES::BLOCKSIZE);
		size_t count = input_fp.gcount();
		if (count == 0) {
			break;
		}
		aes_decryptor.ProcessData(reinterpret_cast<byte*>(buffer), reinterpret_cast<byte*>(buffer), count);
		decrypted_text.append(buffer, count); // Agregar el bloque descifrado al string
	}

	// Escribir el mensaje descifrado en el archivo de salida
	std::ofstream output_fp(output_file, std::ofstream::app);
	if (!output_fp.is_open()) {
		std::cerr << "Error al abrir el archivo de salida" << std::endl;
		exit(EXIT_FAILURE);
	}
	output_fp << decrypted_text;
